...
export default function JobSection({ position }) {
    ...
    useEffect(() => {
        ...
        console.log('🔍 Fetching jobs for position:', position); // debug
        ...
        fetchJobs(position)
            .then(data => {
                console.log('✅ Jobs received:', data); // debug
            ...
            .catch(err => {
                ...
                setError(`Failed to load jobs: ${err.response?.data?.details || err.response?.data?.error || err.message}`);
            })
    ...
    return (
        <div className="bg-gray-900 rounded-2xl p-8 border border-gray-800">
            ...
            {!loading && error && (
                <div className="flex items-start gap-2 text-red-400 bg-red-900/20 border border-red-800 rounded-xl p-4">
            ...
            <div className="space-y-4 mt-4">
                {jobs.map(job => (
                    <div key={job.id} className="bg-gray-800 rounded-xl p-5 flex gap-4 border border-gray-700 hover:bg-gray-700 transition">
                        {job.logo && (
